...
const registerCmd = (cliTool, testplane) => {
    cliTool
        ...
        const { installBrowsersWithDrivers, BrowserInstallStatus } = await Promise.resolve().then(() => __importStar(require("../../../browser-installer")));
        try {
            if (!browsers.length) {
            ...
            logResult(browsersInstallPerStatus, "log", BrowserInstallStatus.Ok, "These browsers are downloaded successfully:", ({ tag }) => `- ${tag}`);
            logResult(browsersInstallPerStatus, "warn", BrowserInstallStatus.Skip, "Browser install for these browsers was skipped:", ({ tag, reason }) => `- ${tag}: ${reason}`);
            logResult(browsersInstallPerStatus, "error", BrowserInstallStatus.Error, "An error occured while trying to download these browsers:", ({ tag, reason }) => `- ${tag}: ${reason}`);
        }
        catch (err) {
