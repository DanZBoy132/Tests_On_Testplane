...
const hasElementRef = (data) => {
    return lodash_1.default.isObject(data) && Object.keys(data).some(key => key.startsWith("element-"));
};
const getElementRef = (elementRefWithExtraProps) => {
    return lodash_1.default.pickBy(elementRefWithExtraProps, (_v, key) => key.startsWith("element-"));
};
class TestRunner extends test_runner_1.default {
    ...
    _waitBroInitOnReconnect() {
        let intervalId = null;
        return (0, promise_1.promiseTimeout)(new Promise((resolve, reject) => {
        ...
        }), constants_1.BRO_INIT_TIMEOUT_ON_RECONNECT, `Browser didn't connect to the Vite server after reconnect in ${constants_1.BRO_INIT_TIMEOUT_ON_RECONNECT}ms`).catch(err => {
            if (intervalId) {
                clearInterval(intervalId);
            }
            throw err;
    ...
    _getPreparePageActions(browser, history) {
        ...
        return [
            async () => {
                ...
                const err = (await this._socket.timeout(constants_2.SOCKET_MAX_TIMEOUT).emitWithAck(types_1.WorkerEventNames.initialize, {
                    file: this._file,
        ...
        return async (payload, cb) => {
            ...
            try {
                ...
                if (lodash_1.default.isArray(result)) {
                    if (result.every((item) => hasElementRef(item))) {
                        const elementRefs = result.map((item) => getElementRef(item));
    ...
    async _openViteUrl(browser) {
        const browserInitialize = new Promise((resolve, reject) => {
            this._socket.once(types_2.BrowserEventNames.initialize, errors => {
        ...
        await Promise.all([
            (0, promise_1.promiseTimeout)(browserInitialize, timeout, `Browser didn't connect to the Vite server in ${timeout}ms`),
