...
const isProtocolCommand = (commandName) => {
        ...
        try {
            ...
            if (!isProtocolCommand(commandName)) {
                ...
                if (Array.isArray(result) && result.length > 0 && result.every(isElementRef)) {
                    const browser = getBrowserInstance(this);
                    return Promise.all(result.map(item => browser.$(item)));
                }
            }
            return result;
        }
...
function getBrowserInstance(ctx) {
    return isWdioElement(ctx) && ctx.parent ? getBrowserInstance(ctx.parent) : ctx;
}
