...
const installChromeBrowser = async (browserName, version, { force = false } = {}) => {
    ...
    const existingLocallyBrowserVersion = registry_1.default.getMatchedBrowserVersion(browserName, platform, version);
    if (existingLocallyBrowserVersion && !force) {
        ...
        return registry_1.default.getBinaryPath(browserName, platform, existingLocallyBrowserVersion);
    }
    const normalizedVersion = (0, utils_2.normalizeChromeVersion)(version);
    const buildId = await (0, browsers_1.resolveBuildId)(browserName, platform, normalizedVersion);
    const cacheDir = (0, utils_1.getBrowsersDir)();
    const canBeInstalled = await (0, browsers_1.canDownload)({ browser: browserName, platform, buildId, cacheDir });
    if (!canBeInstalled) {
        throw new Error([
    ...
    return registry_1.default.installBinary(browserName, platform, buildId, installFn);
};
const installChrome = async (browserName, version, { force = false, needWebDriver = false, needUbuntuPackages = false } = {}) => {
