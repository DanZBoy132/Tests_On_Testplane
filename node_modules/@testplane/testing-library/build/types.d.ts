import { Config as BaseConfig, BoundFunction as BoundFunctionBase, queries, waitForOptions, SelectorMatcherOptions, MatcherOptions } from "@testing-library/dom";
...
export type QueryReturnType<Element, ElementArray, T> = T extends Promise<HTMLElement> ? Element : T extends HTMLElement ? Element : T extends Promise<HTMLElement[]> ? ElementArray : T extends HTMLElement[] ? ElementArray : T extends null ? null : never;
export type BoundFunction<Element, ElementArray, T> = (...params: Parameters<BoundFunctionBase<T>>) => Promise<QueryReturnType<Element, ElementArray, ReturnType<BoundFunctionBase<T>>>>;
export type BoundFunctionSync<Element, ElementArray, T> = (...params: Parameters<BoundFunctionBase<T>>) => QueryReturnType<Element, ElementArray, ReturnType<BoundFunctionBase<T>>>;
...
export type TestplaneQueriesChainable<Container extends SelectorsBase | undefined = SelectorsBase> = {
    [P in keyof Queries as `${string & P}$`]: Container extends SelectorsBase ? BoundFunctionSync<ReturnType<Container["$"]>, ReturnType<Container["$$"]>, Queries[P]> : undefined;
};
