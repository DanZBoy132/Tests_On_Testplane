...
var import_wdio_logger4 = __toESM(require("@testplane/wdio-logger"), 1);
var import_wdio_utils3 = require("@testplane/wdio-utils");
var import_wdio_config = require("@testplane/wdio-config");

// src/devtoolsdriver.ts
var import_uuid3 = require("uuid");
var import_wdio_logger3 = __toESM(require("@testplane/wdio-logger"), 1);
...
var import_import_meta_resolve = require("import-meta-resolve");
var import_wdio_utils = require("@testplane/wdio-utils");
var import_node = require("@testplane/wdio-utils/node");
var import_wdio_protocols = require("@testplane/wdio-protocols");
...
// src/utils.ts
var log = (0, import_wdio_logger.default)("devtools");
...
Usage: ${commandUsage}` + parameterDescription + moreInfo
  ...
  for (const [i, arg] of Object.entries(args2)) {
    ...
    if (!(0, import_wdio_utils.isValidParameter)(arg, commandParam.type)) {
...
Actual: ${(0, import_wdio_utils.getArgumentType)(arg)}` + moreInfo
      );
    }
    body[commandParams[parseInt(i, 10)].name] = arg;
  }
  log.info("COMMAND", (0, import_wdio_utils.commandCallStructure)(command, args2));
  return body;
};
function getPrototype(commandWrapper) {
  const prototype = {};
  for (const [endpoint, methods] of Object.entries(import_wdio_protocols.WebDriverProtocol)) {
...
function findByWhich(executables, priorities) {
  ...
  executables.forEach((executable) => {
    try {
      const browserPath = import_which.default.sync(executable);
      if ((0, import_node.canAccess)(browserPath)) {
...
async function patchDebug(scoppedLogger) {
  const pkgName = "debug";
  try {
    const puppeteerPkg = await (0, import_import_meta_resolve.resolve)("puppeteer-core", __importMetaUrl);
    let puppeteerDebugPkg = await (0, import_import_meta_resolve.resolve)(pkgName, puppeteerPkg);
    if (!await import_promises.default.access(puppeteerDebugPkg).then(() => true, () => false)) {
      puppeteerDebugPkg = await (0, import_import_meta_resolve.resolve)(pkgName, __importMetaUrl);
    }
...
var launchChromeUsingWhich = async (err, launchOptions) => {
  ...
  const chromePath = await (0, import_which.default)("chrome").catch(
    (errorChrome) => (0, import_which.default)("chromium").catch(
      (errorChromium) => (0, import_which.default)("google-chrome").catch((errorGoogleChrome) => {
        throw new Error(import_node_util.default.format(
          errorMessage,
          errorChrome.message,
  ...
  return (0, import_chrome_launcher.launch)({ ...launchOptions, chromePath });
};
...
async function closeWindow() {
  ...
  if (!this.currentWindowHandle) {
    ...
    const newWindowHandle = (0, import_uuid.v4)();
...
var import_wdio_utils2 = require("@testplane/wdio-utils");
async function elementSendKeys({ elementId, text }) {
  ...
  let typeInput = [text];
  for (const [key, value] of Object.entries(import_wdio_utils2.UNICODE_CHARACTERS)) {
  ...
  if (tagName === "INPUT" && type === "file") {
    const paths = (text || "").split("\n").map((p) => import_node_path.default.resolve(p));
    await elementHandle.uploadFile(...paths);
  } else {
    for (const input of typeInput) {
      import_wdio_utils2.UNICODE_CHARACTERS[input] ? await page.keyboard.press(input) : await page.keyboard.type(input);
    }
  }
  return null;
}
...
// src/launcher.ts
var import_chrome_launcher2 = require("chrome-launcher");
var import_puppeteer_core = __toESM(require("puppeteer-core"), 1);
var import_wdio_logger2 = __toESM(require("@testplane/wdio-logger"), 1);
var import_puppeteer = require("query-selector-shadow-dom/plugins/puppeteer/index.js");

// src/finder/edge.ts
var import_node_os = __toESM(require("node:os"), 1);
var import_node_path3 = __toESM(require("node:path"), 1);
var import_node_child_process2 = require("node:child_process");
var import_edge_paths = require("edge-paths");
var import_node3 = require("@testplane/wdio-utils/node");

// src/finder/finder.ts
var import_node_path2 = __toESM(require("node:path"), 1);
var import_node_child_process = require("node:child_process");
var import_node2 = require("@testplane/wdio-utils/node");
var DARWIN_LIST_APPS = "system_profiler SPApplicationsDataType -json";
var darwinGetAppPaths = (app) => {
  const apps = JSON.parse((0, import_node_child_process.execSync)(DARWIN_LIST_APPS).toString());
...
var darwinGetInstallations = (appPaths, suffixes) => {
  ...
  appPaths.forEach((inst) => {
    suffixes.forEach((suffix) => {
      const execPath = import_node_path2.default.join(inst.substring(0, inst.indexOf(".app") + 4).trim(), suffix);
      if ((0, import_node2.canAccess)(execPath) && installations.indexOf(execPath) === -1) {
...
function darwin() {
  ...
  let installations;
  if ((0, import_node3.canAccess)(defaultPath)) {
...
function linux() {
  ...
  const desktopInstallationFolders = [
    import_node_path3.default.join(import_node_os.default.homedir(), ".local/share/applications/"),
...
function win32() {
  const installations = [];
  const suffixes = [
    `${import_node_path3.default.sep}Microsoft${import_node_path3.default.sep}Edge${import_node_path3.default.sep}Application${import_node_path3.default.sep}edge.exe`,
    `${import_node_path3.default.sep}Microsoft${import_node_path3.default.sep}Edge${import_node_path3.default.sep}Application${import_node_path3.default.sep}msedge.exe`,
    `${import_node_path3.default.sep}Microsoft${import_node_path3.default.sep}Edge Dev${import_node_path3.default.sep}Application${import_node_path3.default.sep}msedge.exe`
  ];
  const prefixes = [
  ...
  prefixes.forEach((prefix) => suffixes.forEach((suffix) => {
    const edgePath = import_node_path3.default.join(prefix, suffix);
    checkedPath.push(edgePath);
    if ((0, import_node3.canAccess)(edgePath)) {
      installations.push(edgePath);
    }
  }));
  if (installations.length === 0) {
    const edgePath = (0, import_edge_paths.getEdgePath)();
    if ((0, import_node3.canAccess)(edgePath)) {
      installations.push(edgePath);
    }
  }
...
function findEdgeExecutables(folder) {
  ...
  const installations = [];
  if ((0, import_node3.canAccess)(folder)) {
    let execPaths;
    try {
      execPaths = (0, import_node_child_process2.execSync)(
    ...
    execPaths.forEach((execPath) => (0, import_node3.canAccess)(execPath) && installations.push(execPath));
  }
  return installations;
}
var edge_default = {
...
var import_node4 = require("@testplane/wdio-utils/node");
var newLineRegex2 = /\r?\n/;
function darwin2() {
  const suffixes = [
  ...
  const defaultPath = `/Applications/${appName}.app${suffixes[0]}`;
  let installations;
  if ((0, import_node4.canAccess)(defaultPath)) {
    installations = [defaultPath];
  } else {
    const appPaths = darwinGetAppPaths(appName);
    installations = darwinGetInstallations(appPaths, suffixes);
...
function linux2() {
  let installations = [];
  const desktopInstallationFolders = [
    import_node_path4.default.join(import_node_os2.default.homedir(), ".local/share/applications/"),
    "/usr/share/applications/"
  ];
  desktopInstallationFolders.forEach((folder) => {
...
function win322() {
  const installations = [];
  const suffixes = [
    `${import_node_path4.default.sep}Firefox Nightly${import_node_path4.default.sep}Application${import_node_path4.default.sep}firefox.exe`
  ];
  const prefixes = [
    process.env.LOCALAPPDATA || "",
    process.env.PROGRAMFILES || "",
  ...
  prefixes.forEach((prefix) => suffixes.forEach((suffix) => {
    const firefoxPath = import_node_path4.default.join(prefix, suffix);
    if ((0, import_node4.canAccess)(firefoxPath)) {
...
function findFirefoxExecutables(folder) {
  ...
  const installations = [];
  if ((0, import_node4.canAccess)(folder)) {
    let execPaths;
    try {
      execPaths = (0, import_node_child_process3.execSync)(
        `grep -ER "${edgeExecRegex}" ${folder} | awk -F '=' '{print $2}'`,
        { stdio: "pipe" }
      );
    } catch (err) {
    ...
    execPaths.forEach((execPath) => (0, import_node4.canAccess)(execPath) && installations.push(execPath));
  }
  return installations;
}
var firefox_default = {
...
// src/launcher.ts
var log2 = (0, import_wdio_logger2.default)("devtools");
var DEVICE_NAMES = Object.keys(import_puppeteer_core.KnownDevices);
async function launchChrome(capabilities) {
  ...
  if (typeof mobileEmulation.deviceName === "string") {
    const deviceProperties = import_puppeteer_core.KnownDevices[mobileEmulation.deviceName];
    if (!deviceProperties) {
  ...
  const chrome = await (0, import_chrome_launcher2.launch)(launchOptions).catch(
...
function launchBrowser(capabilities, browserType) {
  ...
  return import_puppeteer_core.default.launch(puppeteerOptions);
}
function connectBrowser(connectionUrl, capabilities) {
  ...
  return import_puppeteer_core.default.connect(options);
}
async function launch(capabilities) {
  try {
    import_puppeteer_core.Puppeteer.unregisterCustomQueryHandler("shadow");
  } catch {
  }
  import_puppeteer_core.Puppeteer.registerCustomQueryHandler("shadow", import_puppeteer.QueryHandler);
...
async function newSession({ capabilities }) {
  ...
  const sessionId = (0, import_uuid2.v4)();
  ...
  return {
    ...
    capabilities: {
      browserName,
      browserVersion,
      platformName: import_node_os3.default.platform(),
      platformVersion: import_node_os3.default.release()
    }
  };
}

// src/commands/performActions.ts
var import_USKeyboardLayout = require("puppeteer-core/lib/esm/puppeteer/common/USKeyboardLayout.js");
var KEY = "key";
var POINTER = "pointer";
  ...
  for (const action of actions) {
    ...
    if (action.type === "key") {
      ...
      for (const singleAction of action.actions) {
        ...
        const pptrKey = key && import_USKeyboardLayout._keyDefinitions[key] ? key : import_USKeyboardLayout._keyDefinitions[singleAction.value] ? singleAction.value : void 0;
        if (!pptrKey) {
...
var import_import_meta_resolve2 = require("import-meta-resolve");
var puppeteerVersion;
async function status() {
  if (!puppeteerVersion) {
    const puppeteerPath = await (0, import_import_meta_resolve2.resolve)("puppeteer-core", __importMetaUrl);
    try {
      const pkgJsonPath = import_node_path5.default.resolve(import_node_url.default.fileURLToPath(puppeteerPath), "..", "..", "..", "..", "package.json");
      const pkgJson = JSON.parse((await import_promises2.default.readFile(pkgJsonPath, "utf-8")).toString());
...
// src/devtoolsdriver.ts
var log3 = (0, import_wdio_logger3.default)("devtools");
var DevToolsDriver = class {
  ...
  _createWindowHandle(page) {
    const pageId = (0, import_uuid3.v4)();
...
// src/index.ts
var log4 = (0, import_wdio_logger4.default)("devtools:puppeteer");
...
var DevTools = class {
  static async newSession(options, modifier, userPrototype = {}, customCommandWrapper) {
    ...
    const params = (0, import_wdio_config.validateConfig)(DEFAULTS, options);
    if (params.logLevel && (!options.logLevels || !options.logLevels.devtools)) {
      import_wdio_logger4.default.setLevel("devtools", params.logLevel);
    }
    if (params.outputDir && !process.env.WDIO_LOG_PATH) {
      process.env.WDIO_LOG_PATH = import_node_path6.default.join(params.outputDir, "wdio.log");
    }
    ...
    const sessionId = (0, import_uuid4.v4)();
    const uaParser = new import_ua_parser_js.default(await browser.userAgent());
    ...
    params.capabilities = {
      ...
      platformName: import_node_os4.default.platform()
      // platform: os.release()
    };
    if (vendorCapPrefix) {
    ...
    Object.entries((0, import_wdio_utils3.devtoolsEnvironmentDetector)({
    ...
    const monad = (0, import_wdio_utils3.webdriverMonad)(
