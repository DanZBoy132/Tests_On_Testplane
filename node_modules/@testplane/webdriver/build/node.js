...
var WebDriverRequest = class extends EventEmitter {
  ...
  async _createOptions(options, sessionId, isBrowser = false) {
    ...
    requestOptions.url = new URL2(`${options.protocol}://${options.hostname}:${options.port}${this.isHubCommand ? this.endpoint : path.join(options.path || "", endpoint)}`);
  ...
  async _request(fullRequestOptions, transformResponse, totalRetryCount = 0, retryCount = 0) {
    log5.info(`[${fullRequestOptions.method}] ${fullRequestOptions.url.href}`);
    ...
    const retry = (error2, msg) => {
      if (retryCount >= totalRetryCount || error2.message.includes("invalid session id")) {
        log5.error(`Request failed with status ${response.statusCode} due to ${error2}`);
...
var NodeJSRequest = class extends WebDriverRequest {
  constructor(method, endpoint, body, isHubCommand = false) {
  ...
  async _libRequest(url, options) {
    ...
    for (const opt of Object.keys(options)) {
      if (typeof options[opt] !== "undefined" && UNSUPPORTED_OPTS.includes(opt) && options[opt] !== this.defaultOptions[opt]) {
        log6.info(`Browser-based webdriver does not support the '${String(opt)}' option; behavior may be unexpected`);
        continue;
      }
...
async function connectWebsocket(candidateUrls, options) {
  const websockets = candidateUrls.map((candidateUrl) => {
    log7.debug(`Attempt to connect to webSocketUrl ${candidateUrl}`);
    try {
  ...
  const wsConnectPromises = websockets.map((ws2, index) => {
    const promise = new Promise((resolve) => {
      ...
      ws2.once("error", (err) => {
        log7.debug(`Could not connect to Bidi protocol at ${candidateUrls[index]}: ${err.message}`);
  ...
  const connectionTimeoutPromise = new Promise((resolve) => {
    setTimeout(() => {
      log7.error(`Could not connect to Bidi protocol of any candidate url in time: "${candidateUrls.join('", "')}"`);
  ...
  if (wsInfo?.isConnected) {
    log7.info(`Connected to Bidi protocol at ${candidateUrls[wsInfo.index]}`);
    return wsInfo.ws;
  }
  return void 0;
}
function firstResolved(promises, errorMessages = []) {
  if (promises.length === 0) {
    ...
    log7.error("Could not connect to Bidi protocol" + errorMessage);
...
environment.value = {
  Request: (
     ...
     */
    process.env.WDIO_UNIT_TESTS ? WebRequest : NodeJSRequest
  ),
  Socket: ws,
