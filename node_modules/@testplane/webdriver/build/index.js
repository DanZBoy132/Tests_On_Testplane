...
import { WebDriverBidiProtocol } from "@testplane/wdio-protocols";
...
async function startWebDriverSession(params) {
  ...
  if (
     ...
     */
    typeof w3cCaps.alwaysMatch.browserName === "string" && w3cCaps.alwaysMatch.browserName.toLowerCase() !== "safari"
  ) {
...
function getPrototype({ isW3C, isChromium, isFirefox, isMobile, isSauce, isSeleniumStandalone }) {
  ...
  const ProtocolCommands = deepmerge(
     ...
     */
    isMobile ? deepmerge(JsonWProtocol, WebDriverProtocol) : isW3C ? WebDriverProtocol : JsonWProtocol,
    /**
...
var getTimeoutError = (error, requestOptions) => {
  const cmdName = getExecCmdName(requestOptions);
  const cmdArgs = getExecCmdArgs(requestOptions);
  const cmdInfoMsg = 'when running "'.concat(cmdName, '" with method "').concat(requestOptions.method, '"');
  const cmdArgsMsg = cmdArgs ? " and args ".concat(cmdArgs) : "";
  const timeoutErr = new Error("".concat(error.message, " ").concat(cmdInfoMsg).concat(cmdArgsMsg));
...
import { WebDriverProtocol as WebDriverProtocol2 } from "@testplane/wdio-protocols";
import { URL } from "node:url";
import logger5 from "@testplane/wdio-logger";
import { transformCommandLogResult as transformCommandLogResult2 } from "@testplane/wdio-utils";
...
var WebDriverRequest = class extends EventEmitter {
  constructor(method, endpoint, body, isHubCommand = false) {
    ...
    __publicField(this, "defaultAgents");
  ...
  async _createOptions(options, sessionId, isBrowser = false) {
    const agent = isBrowser ? void 0 : options.agent || this.defaultAgents;
    const searchParams = isBrowser ? void 0 : typeof options.queryParams === "object" ? options.queryParams : {};
    ...
    if (this.body && (Object.keys(this.body).length || this.method === "POST")) {
      ...
      requestOptions.headers["Content-Length"] = "".concat(contentLength);
    }
    ...
    requestOptions.url = new URL("".concat(options.protocol, "://").concat(options.hostname, ":").concat(options.port).concat(this.isHubCommand ? this.endpoint : path.join(options.path || "", endpoint)));
    if (this.endpoint === "/session" && options.user && options.key) {
      requestOptions.username = options.user;
      requestOptions.password = options.key;
    }
    requestOptions.https.rejectUnauthorized = !(options.strictSSL === false || process.env.STRICT_SSL === "false" || process.env.strict_ssl === "false");
  ...
  async _request(fullRequestOptions, transformResponse, totalRetryCount = 0, retryCount = 0) {
    log5.info("[".concat(fullRequestOptions.method, "] ").concat(fullRequestOptions.url.href));
    if (fullRequestOptions.json && Object.keys(fullRequestOptions.json).length) {
      log5.info("DATA", transformCommandLogResult2(fullRequestOptions.json));
    }
    ...
    const retry = (error2, msg) => {
      if (retryCount >= totalRetryCount || error2.message.includes("invalid session id")) {
        log5.error("Request failed with status ".concat(response.statusCode, " due to ").concat(error2));
    ...
    if (response instanceof Error) {
      if (response.code === "ETIMEDOUT") {
        const error2 = getTimeoutError(response, fullRequestOptions);
    ...
    return retry(error, "Request failed with status ".concat(response.statusCode, " due to ").concat(error.message));
  }
};
function findCommandPathByName(commandName) {
  ...
  if (!command) {
    throw new Error("Couldn't find command \"".concat(commandName, '"'));
  }
  return command[0];
}
...
var WebRequest = class extends WebDriverRequest {
  constructor(method, endpoint, body, isHubCommand = false) {
  ...
  async _libRequest(url, options) {
    ...
    for (const opt of Object.keys(options)) {
      if (typeof options[opt] !== "undefined" && UNSUPPORTED_OPTS.includes(opt) && options[opt] !== this.defaultOptions[opt]) {
        log6.info("Browser-based webdriver does not support the '".concat(String(opt), "' option; behavior may be unexpected"));
        continue;
      }
      kyOptions[opt] = options[opt];
    }
    if (options.username && options.password) {
      const encodedAuth = Buffer.from("".concat(options.username, ":").concat(options.password), "utf8").toString("base64");
      kyOptions.headers = {
        ...kyOptions.headers,
...
environment.value = {
  ...
  createBidiConnection: (webSocketUrl, options) => {
    log7.info("Connecting to webSocketUrl ".concat(webSocketUrl));
    const ws = new BrowserSocket(webSocketUrl, options);
    return new Promise((resolve) => {
      ws.on("open", () => {
        log7.info("Connected session to Bidi protocol");
        resolve(ws);
      });
      ws.on("error", (err) => {
        const error = err instanceof Error ? err : new Error(String(err));
        log7.warn("Couldn't connect to Bidi protocol: ".concat(error.message));
        resolve(void 0);
      });
    });
  },
