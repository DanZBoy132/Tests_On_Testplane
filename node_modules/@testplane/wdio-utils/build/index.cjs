...
__export(index_exports, {
  ...
  asyncIterators: () => pIteration_exports,
  capabilitiesEnvironmentDetector: () => capabilitiesEnvironmentDetector,
  commandCallStructure: () => commandCallStructure,
  devtoolsEnvironmentDetector: () => devtoolsEnvironmentDetector,
  enableFileLogging: () => enableFileLogging,
  executeAsync: () => executeAsync,
  executeHooksWithArgs: () => executeHooksWithArgs,
  getArgumentType: () => getArgumentType,
  getBrowserObject: () => getBrowserObject,
  initializeLauncherService: () => initializeLauncherService,
  initializePlugin: () => initializePlugin,
  initializeWorkerService: () => initializeWorkerService,
  isAppiumCapability: () => isAppiumCapability,
  isBidi: () => isBidi,
  isFunctionAsync: () => isFunctionAsync,
  isValidParameter: () => isValidParameter,
  isW3C: () => isW3C,
  safeImport: () => safeImport,
  sessionEnvironmentDetector: () => sessionEnvironmentDetector,
  sleep: () => sleep,
  startWebDriver: () => startWebDriver,
  testFnWrapper: () => testFnWrapper,
  transformCommandLogResult: () => transformCommandLogResult,
  userImport: () => userImport,
  webdriverMonad: () => WebDriver,
  wrapCommand: () => wrapCommand,
  wrapGlobalTestMethod: () => wrapGlobalTestMethod
});
...
// src/monad.ts
var import_wdio_types = require("@testplane/wdio-types");
...
async function safeImport(name) {
  ...
  try {
    if (!globalThis.window) {
      ...
      try {
        importPath = await resolve(name, __importMetaUrl);
      } catch {
        const localNodeModules = import_node_path.default.join(process.cwd(), "node_modules");
        try {
          importPath = await resolve(name, import_node_url.default.pathToFileURL(localNodeModules).toString());
        } catch {
          return null;
        }
      }
...
async function enableFileLogging(outputDir) {
  ...
  await import_promises.default.mkdir(import_node_path.default.join(outputDir), { recursive: true });
  process.env.WDIO_LOG_PATH = import_node_path.default.join(outputDir, "wdio.log");
}
...
function WebDriver(options, modifier, propertiesObject = {}) {
  ...
  const log4 = logger_default("webdriver");
  const eventHandler = new import_node_events.EventEmitter();
    ...
    client.addCommand = function(name, func, attachToElement = false, proto, instances) {
      ...
      if (typeof process.send === "function" && process.env.WDIO_WORKER_ID) {
        const message = {
          ...
          name: "workerEvent",
          args: {
            type: import_wdio_types.MESSAGE_TYPES.customCommand,
            value: {
              commandName: name,
...
var log = logger_default("@testplane/wdio-utils");
...
var log2 = logger_default("@testplane/wdio-utils:initializeServices");
...
// src/shim.ts
var log3 = logger_default("@testplane/wdio-utils:shim");
...
function isW3C(capabilities) {
  ...
  const isAppium = Boolean(
    ...
    capabilities.automationName || capabilities["appium:automationName"] || capabilities.deviceName || capabilities["appium:deviceName"] || capabilities.appiumVersion || capabilities["appium:appiumVersion"]
  );
  const hasW3CCaps = Boolean(
     ...
     */
    (capabilities.platformVersion || capabilities["appium:platformVersion"] || Object.prototype.hasOwnProperty.call(capabilities, "setWindowRect"))
  );
