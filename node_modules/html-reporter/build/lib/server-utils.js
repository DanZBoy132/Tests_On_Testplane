...
const getTempPath = async (destPath) => {
    const { default: tmp } = await Promise.resolve().then(() => __importStar(require('tmp')));
    return path_1.default.resolve(tmp.tmpdir, destPath);
};
...
async function copyFilesToReportDir(destPath, ext, sourceDirectory) {
    const sourceDirectoryEntries = await fs_extra_1.default.readdir(sourceDirectory);
    ...
    return Promise.all(extensionFilteredEntries.map(async (entry) => {
        const from = path_1.default.resolve(sourceDirectory, entry);
        const to = path_1.default.resolve(destPath, entry);
        const isFile = await fs_extra_1.default.stat(from).then(stat => stat.isFile());
        if (isFile) {
            await fs_extra_1.default.copy(from, to);
        }
    }));
}
exports.copyFilesToReportDir = copyFilesToReportDir;
...
const formatTestResult = (rawResult, status, attempt = constants_1.UNKNOWN_ATTEMPT, saveHistoryMode) => {
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    const { TestplaneTestResultAdapter } = require('./adapters/test-result/testplane');
    return new TestplaneTestResultAdapter(rawResult, { attempt, status, duration: rawResult.duration, saveHistoryMode });
};
