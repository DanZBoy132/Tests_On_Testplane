...
class StaticReportBuilder {
    ...
    async addTestResult(formattedResultOriginal) {
        ...
        const isImgSkipped = testResultWithImagePaths.status === constants_1.SKIPPED;
        const isPreviouslySkippedTest = isImgSkipped && (0, server_1.getTestFromDb)(this._dbClient, formattedResult);
        if (!ignoredStatuses.includes(testResultWithImagePaths.status) && !isPreviouslySkippedTest) {
            if (typeof this._reporterConfig.generateBadges === 'function' && testResultWithImagePaths.attachments) {
                testResultWithImagePaths.attachments.push({
                    type: types_1.AttachmentType.Badges,
                    list: this._reporterConfig.generateBadges(testResultWithImagePaths)?.filter((badge) => badge && badge.title)
                });
            }
