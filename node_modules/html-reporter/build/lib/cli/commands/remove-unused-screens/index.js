...
module.exports = (program, toolAdapter) => {
    ...
    program
        ...
        .action((options) => removeUnusedScreens(options, toolAdapter));
};
async function removeUnusedScreens(options, toolAdapter) {
    const { toolName } = toolAdapter;
    if (toolName !== ToolName.Testplane) {
        throw new Error(`CLI command "${commandName}" supports only "${ToolName.Testplane}" tool`);
    }
    const ora = require('ora');
    const { getTestsFromFs, findScreens, askConfirm } = require('./utils');
    try {
        proxyTool();
        ...
        const shouldIdentifyOutdated = await askConfirm({
        ...
        const shouldIdentifyUnused = await askConfirm({
...
async function handleOutdatedScreens(screenPaths, screenPatterns, opts = {}) {
    const { identifyOutdatedScreens } = require('./utils');
    const { spinner } = opts;
...
async function handleUnusedScreens(screenPaths, fsTests, opts = {}) {
    const { identifyUnusedScreens } = require('./utils');
...
async function handleScreens(allScreenPaths, screensToRemove, { spinner, cliOpts } = {}) {
    ...
    const { removeScreens, askConfirm } = require('./utils');
    ...
    const shouldShowList = await askConfirm({
