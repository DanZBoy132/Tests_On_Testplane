...
exports.findScreens = async (screenPatterns) => {
    const fg = require('fast-glob');
    const globPatterns = await Promise.all(screenPatterns.map(pattern => fg(pattern)));
    return _.uniq(_.flatten(globPatterns));
};
exports.askConfirm = async (question, { skipQuestions } = {}) => {
    if (skipQuestions) {
        return question.default;
    }
    return require('@inquirer/confirm').default(question);
};
